 ...
 * to create three-dimensional models, simulations, and games. Here is a
 * <a href = "http://introcs.cs.princeton.edu/java/stddraw3d">StdDraw3D
 * tutorial</a> and the <a href =
 * "http://introcs.cs.princeton.edu/java/stddraw3d/stddraw3d-manual.html">StdDraw3D
 * reference manual</a>.
 */
...
public final class StdDraw3D implements MouseListener, MouseMotionListener, MouseWheelListener, KeyListener,
	...
	private static void initializeCanvas() {
		...
		canvas = new Canvas3D(config) {
			...
			public void postRender() {
				 ...
				 * while (!showedOnce) {
				 * 
				 * try { Thread.currentThread().sleep(0); } catch (InterruptedException e) {
			 ...
			 * 
			 * try { Thread.currentThread().sleep(5); } catch (InterruptedException e) {
			 * System.out.println("Error sleeping"); } } //if (!showedOnce) { // try {
			 * Thread.currentThread().sleep(30); } // catch (InterruptedException e) {
			 * System.out.println("Error sleeping"); } //}
			 * //StdOut.println("showed once is false, rendered once"); showedOnce = false;
			 * //renderedOnce = true;
			 * 
			 * //Thread.yield(); }
			 */
	...
	/**
	 * The camera can be controlled with the static functions already listed in this
	 * reference. However, much more advanced control of the camera can be obtained
	 * by manipulating the Camera object. The Camera is basically equivalent to a
	 * Shape with a couple of extra functions, so it can be moved and rotated just
	 * like a Shape. This functionality works well with point-of-view simulations
	 * and games.
	 */
	...
	/**
	 * Everything three-dimensional you draw in your scene from spheres to points to
	 * 3D text is actually a Shape object. When you call a drawing function, it
	 * always returns a Shape object. If you keep a reference to this object, you
	 * can manipulate the already drawn Shape instead of clearing and redrawing it,
	 * which is a much more powerful and more efficient method of animation.
	 */
	...
	public static class Vector3D {
		 ...
		 * 
		 * this.x = min + Math.random() * (max - min); this.y = min + Math.random() *
		 * (max - min); this.z = min + Math.random() * (max - min); }
		 */
		// --------------------------------------------------------------------------
		/**
